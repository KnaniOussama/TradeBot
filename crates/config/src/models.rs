...
impl WhaleEntry {
    pub fn validate(&self) -> Result<(), ConfigError> {
        if !(32..=64).contains(&self.address.len()) {
            return Err(invalid(format!(
                "whale address length out of range: {}",
                self.address
            )));
        }
...
fn default_per_wallet_swap_limit() -> u32 {
    20
}
...
pub struct RiskConfig {
    #[serde(default = "ri_max_concurrent")]
    pub max_concurrent_positions: u32,
    #[serde(default = "ri_size_min")]
    pub per_trade_size_min: f64,
    #[serde(default = "ri_size_max")]
    pub per_trade_size_max: f64,
    #[serde(default = "ri_trailing")]
    pub trailing_stop_pct: f64,
    #[serde(default = "ri_take_profit")]
    pub take_profit_pct: f64,
    #[serde(default = "ri_daily")]
    pub daily_loss_limit_pct: f64,
    #[serde(default = "ri_weekly")]
    pub weekly_loss_limit_pct: f64,
    #[serde(default = "ri_kill")]
    pub per_trade_kill_pct: f64,
    #[serde(default = "ri_drawdown")]
    pub drawdown_circuit_pct: f64,
    #[serde(default = "ri_slippage")]
    pub max_slippage_pct: f64,
    #[serde(default = "ri_max_trades")]
    pub max_trades_per_day: u32,
    #[serde(default = "ri_tp_ladder_pct")]
    pub tp_ladder_pct: f64,
    #[serde(default = "ri_tp_ladder_frac")]
    pub tp_ladder_fraction: f64,
    #[serde(default = "ri_true")]
    pub regime_filter_enabled: bool,
    #[serde(default = "ri_true")]
    pub regime_block_chop: bool,
    #[serde(default = "ri_true")]
    pub use_kelly_sizing: bool,
}

fn ri_max_concurrent() -> u32 {
    3
}
fn ri_size_min() -> f64 {
    0.30
...
fn ri_max_trades() -> u32 {
    10
}
fn ri_tp_ladder_pct() -> f64 {
    0.02
}
fn ri_tp_ladder_frac() -> f64 {
    0.5
}
fn ri_true() -> bool {
    true
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        for (name, map) in [("timeframes", &self.timeframes), ("signals", &self.signals)] {
            ...
            if (total - 1.0).abs() > 1e-6 {
                return Err(invalid(format!(
                    "{name} weights must sum to 1.0, got {total}"
                )));
            }
        }
        Ok(())
...
pub struct AppConfig {
    ...
    #[serde(default = "app_log_level")]
    pub log_level: String,
    #[serde(default = "app_starting_capital")]
    pub starting_capital_usd: f64,
    #[serde(default = "app_jupiter_base_url")]
    pub jupiter_base_url: String,
    #[serde(default = "app_price_poll")]
    pub price_poll_interval_s: f64,
    #[serde(default = "app_decision_interval")]
    pub decision_interval_s: f64,
    #[serde(default = "app_jup_rps")]
    pub jupiter_rate_limit_rps: f64,
    #[serde(default = "app_jup_burst")]
    pub jupiter_rate_limit_burst: u32,
    #[serde(default = "app_jup_retries")]
    pub jupiter_max_429_retries: u32,
    #[serde(default = "app_entry_threshold")]
    pub entry_threshold: f64,
    #[serde(default = "app_exit_flip")]
    pub exit_flip_threshold: f64,
    #[serde(default = "app_helius_base_url")]
    pub helius_base_url: String,
    #[serde(default)]
    pub onchain_dex_addresses: Vec<String>,
    #[serde(default = "app_onchain_whale_min")]
    pub onchain_whale_min: f64,
    #[serde(default)]
    pub birdeye_api_key_env: String,
    #[serde(default = "app_birdeye_base_url")]
    pub birdeye_base_url: String,
    #[serde(default = "app_birdeye_rps")]
    pub birdeye_rate_limit_rps: f64,
    #[serde(default = "app_birdeye_burst")]
    pub birdeye_rate_limit_burst: u32,
    #[serde(default = "app_birdeye_retries")]
    pub birdeye_max_429_retries: u32,
    #[serde(default)]
    pub fast_tick_interval_s: f64,
    #[serde(default = "app_sim_fee_bps")]
    pub simulated_fee_bps: u32,
    #[serde(default = "ri_true")]
    pub dashboard_enabled: bool,
    #[serde(default = "app_dash_host")]
    pub dashboard_host: String,
    #[serde(default = "app_dash_port")]
    pub dashboard_port: u16,
    #[serde(default = "app_keystore_path")]
    pub keystore_path: String,
    #[serde(default)]
    pub priority_fee_microlamports: u64,
    #[serde(default = "app_confirmation_timeout")]
    pub confirmation_timeout_s: f64,
    #[serde(default = "app_starting_sol")]
    pub starting_sol_balance: f64,
    #[serde(default = "app_sim_confirm_latency")]
    pub simulated_confirm_latency_s: f64,
}

fn app_log_level() -> String {
    "INFO".into()
}
fn app_starting_capital() -> f64 {
    50.0
}
fn app_jupiter_base_url() -> String {
    "https://lite-api.jup.ag/swap/v1".into()
}
fn app_price_poll() -> f64 {
    5.0
}
fn app_decision_interval() -> f64 {
    10.0
}
fn app_jup_rps() -> f64 {
    0.9
}
fn app_jup_burst() -> u32 {
    5
}
fn app_jup_retries() -> u32 {
    3
}
fn app_entry_threshold() -> f64 {
    0.6
}
fn app_exit_flip() -> f64 {
    -0.3
}
fn app_helius_base_url() -> String {
    "https://api.helius.xyz".into()
}
fn app_onchain_whale_min() -> f64 {
    1000.0
}
fn app_birdeye_base_url() -> String {
    "https://public-api.birdeye.so".into()
}
fn app_birdeye_rps() -> f64 {
    0.9
}
fn app_birdeye_burst() -> u32 {
    2
}
fn app_birdeye_retries() -> u32 {
    2
}
fn app_sim_fee_bps() -> u32 {
    10
}
fn app_dash_host() -> String {
    "127.0.0.1".into()
}
fn app_dash_port() -> u16 {
    8765
}
fn app_keystore_path() -> String {
    "keystore/bot.keystore.json".into()
}
fn app_confirmation_timeout() -> f64 {
    30.0
}
fn app_starting_sol() -> f64 {
    0.05
}
fn app_sim_confirm_latency() -> f64 {
    1.0
}
...
mod tests {
    ...
    fn whale_entry_defaults_label_to_empty() {
        let w: WhaleEntry =
            serde_json::from_str(&format!("{{\"address\":\"{}\"}}", mint43())).unwrap();
        assert_eq!(w.label, "");
    }
    ...
    fn risk_config_rejects_inverted_size_range() {
        let r = RiskConfig {
            per_trade_size_min: 0.6,
            per_trade_size_max: 0.4,
            ..RiskConfig::default()
        };
